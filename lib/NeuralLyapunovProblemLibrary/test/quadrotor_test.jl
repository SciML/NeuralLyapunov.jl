...
#################################### Hovering quadrotor ####################################
@testset "3D quadrotor vertical only test" begin
    ...
    quadrotor_3d_simplified = mtkcompile(
        quadrotor_3d;
        inputs = inputs(quadrotor_3d),
        outputs = [],
        simplify = true,
        split = false
    )
    ...
    prob = ODEProblem(quadrotor_3d_vertical_only, merge(x0, p), 15τ)
...
@testset "3D quadrotor LQR test" begin
    ...
    u = map(
        Base.Fix1(getproperty, quadrotor_3d),
        toexpr.(getproperty.(inputs(quadrotor_3d), :f))
    )
    x = map(
        Base.Fix1(getproperty, quadrotor_3d),
        toexpr.(getproperty.(x, :f))
    ...
    prob = ODEProblem(quadrotor_3d_lqr, merge(x0, p), 15τ)
    sol = solve(prob, Tsit5())
