...
@testset "Undriven double pendulum energy conservation test" begin
    ...
    prob = ODEProblem(mtkcompile(double_pendulum_undriven), merge(x0, p), 100)
...
@testset "Double pendulum feedback cancellation test" begin
    ...
    function π_cancellation(x, p)
        ...
        M = [I1+I2+m2*l1^2+2*m2*l1*lc2*cos(θ2) I2+m2 * l1 * lc2 * cos(θ2);
            I2+m2 * l1 * lc2 * cos(θ2) I2]
        C = [-2*m2*l1*lc2*sin(θ2)*ω2 -m2*l1*lc2*sin(θ2)*ω2;
            m2*l1*lc2*sin(θ2)*ω1 0]
        G = [-m1 * g * lc1 * sin(θ1) - m2 * g * (l1 * sin(θ1) + lc2 * sin(θ1 + θ2));
            -m2 * g * lc2 * sin(θ1 + θ2)]
        ζ = 2.0
        ω0 = 1.0
        q̈_desired = -2ζ * ω0 * ω - ω0^2 * sin.(q .- [π, π])
        return M * q̈_desired + C * ω - G
    end

    double_pendulum_simplified = mtkcompile(
        double_pendulum;
        inputs = inputs(double_pendulum),
        outputs = [],
        simplify = true,
        split = false
    )
    ...
    @named cancellation_controller = System(u .~ π_cancellation(x, p), t, vcat(x, u), p)
    ...
    x0 = Dict(x .=> vcat(0.01 * rand(rng, 2) .+ π, zeros(2)))

    prob = ODEProblem(double_pendulum_feedback_cancellation, merge(x0, p), 100)
...
@testset "DoublePendulum LQR test" begin
    println("DoublePendulum LQR test")

    function double_pendulum_lqr_matrix(p; x_eq = [π, π, 0, 0], Q = I(4), R = I(2))
        I1, I2, l1, l2, lc1, lc2, m1, m2, g = p
        θ1, θ2 = x_eq[1:2]
    ...
    function π_lqr(p; x_eq = [π, π, 0, 0], Q = I(4), R = I(2))
        L = double_pendulum_lqr_matrix(p; x_eq, Q, R)
        return (x) -> -L * (x .- x_eq)
    end
    ...
    params = map(Base.Fix1(getproperty, double_pendulum), toexpr.(parameters(double_pendulum)))
    u = map(
        Base.Fix1(getproperty, double_pendulum),
        toexpr.(getproperty.(inputs(double_pendulum_simplified), :f))
    )
    ...
    @named double_pendulum_lqr = compose(lqr_controller, double_pendulum)
    double_pendulum_lqr = mtkcompile(double_pendulum_lqr)

    # Remain close to upward equilibrium
    x0 = [π, π, 0, 0] + 0.005 * vcat(2π * rand(rng, 2) .- π, 2 * rand(rng, 2) .- 1)
    tspan = 1000

    prob = ODEProblem(double_pendulum_lqr, Dict(vcat(x, params) .=> vcat(x0, p)), tspan)
    sol = solve(prob, Tsit5())

    anim = plot_double_pendulum(
        sol, p; angle1_symbol = :double_pendulum₊θ1, angle2_symbol = :double_pendulum₊θ2)
    @test anim isa Plots.Animation
    # gif(anim, fps=50)

    x1_end, y1_end = sin(sol[double_pendulum.θ1][end]), -cos(sol[double_pendulum.θ1][end])
    ω1_end = sol[Dt(double_pendulum.θ1)][end]
    x2_end, y2_end = sin(sol[double_pendulum.θ2][end]), -cos(sol[double_pendulum.θ2][end])
    ω2_end = sol[Dt(double_pendulum.θ2)][end]
    @test sqrt(sum(abs2, [x1_end, y1_end] .- [0, 1]))≈0 atol=1e-4
    @test sqrt(sum(abs2, [x2_end, y2_end] .- [0, 1]))≈0 atol=1e-4
    @test ω1_end≈0 atol=1e-4
    @test ω2_end≈0 atol=1e-4
...
################################## LQR acrobot controller ##################################
@testset "Acrobot LQR test" begin
    ...
    acrobot_simplified = mtkcompile(
        acrobot;
        inputs = inputs(acrobot),
        outputs = [],
        simplify = true,
        split = false
    )
    ...
    prob = ODEProblem(acrobot_lqr, Dict(vcat(x, params) .=> vcat(x0, p)), tspan)
    sol = solve(prob, Tsit5())
