...
V̇_func(states::AbstractMatrix) = reshape(map(x->x[1]⋅x[2], zip(eachslice(SHO_dynamics(states), dims=2), eachslice(∇V_func(states), dims=2))), (1,:))
...
println("V ∋ [", min(first(V_func([0.,0.])), minimum(V_predict)), ", ", maximum(V_predict), "]")
println("V̇ ∋ [", minimum(dVdt_predict), ", ", max(first(V̇_func([0.,0.])), maximum(dVdt_predict)), "]")
