...
eqs = [ 
        max(0, V̇_sym(x, y) + λ_sym(x,y) * (V_sym(x,y) - 1) ) ~ 0.,
...
p1 = scatter!([-pi, pi], [0., 0.], label="Unstable equilibria");
p1 = scatter!([-2*pi, 0., 2*pi], [0., 0., 0.], label="Stable equilibria");
...
p4 = plot(xs, ys, dVdt_predict, linetype=:contourf, title="dV/dt", xlabel="x", ylabel="ẋ"; seriescolor=:bluesreds);
p4 = quiver!(XS, YS, quiver=(0.2*fx_vals, 0.2*fy_vals), color=:black);
p4 = scatter!([-pi, pi], [0., 0.], label="Unstable equilibria", color=:red);
p4 = scatter!([-2*pi, 0., 2*pi], [0., 0., 0.], label="Stable equilibria", xlims=[minimum(xs), maximum(xs)], ylims=[minimum(ys), maximum(ys)], color=:green)
